...
```
    ...
    6）、注册一个ApplicationListenerDetector；来在Bean创建完成后检查是否是			 		   			 ApplicationListener，如果是
...
1、beanFactory.preInstantiateSingletons();初始化后剩下的单实例bean
		...
		3）、Bean不是抽象的，是单实例的，是懒加载；
			...
			2）、不是工厂Bean。利用getBean(beanName);创建对象
				...
				2、先获取缓存中保存的单实例Bean。如果能获取到说明这个Bean之前被创建过（所有创建过的单 				   实例Bean都会被缓存起来）
				...
				7、启动单实例Bean的创建流程；
					...
					5）、将创建的Bean添加到缓存中singletonObjects；
			 			...
			 			检查所有的Bean是否是SmartInitializingSingleton接口的；如果是；就执行 							afterSingletonsInstantiated()；
```
